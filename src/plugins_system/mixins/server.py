...
from typing import (
    TYPE_CHECKING, Any, Iterable, List, TypeAlias, Dict, Callable, Union, cast, Final, Optional
)
...
class ServiceResult:
    ...
    @classmethod
    def success_result(cls, data: Any = None, execution_time: float = 0.0) -> ServiceResult:
...
def _create_service_handler(
    ...
    def _create_meta(e: Event, start_time: float) -> ServiceMeta:
        """创建元数据对象"""
        caller_info = CallerInfo(
            plugin_name=service_info.owner,
            source=e.source,
    ...
    def _inject_meta_if_needed(kwargs: Dict[str, Any], meta: ServiceMeta) -> Dict[str, Any]:
        """如果需要，注入meta参数"""
    ...
    async def _acall_with_meta(args: tuple, kwargs: Dict[str, Any], meta: ServiceMeta) -> Any:
        """使用meta参数调用异步函数"""
        final_kwargs = _inject_meta_if_needed(kwargs, meta)
    ...
    if inspect.iscoroutinefunction(func):
        ...
        async def _async_event_handler(e: Event) -> ServiceResult:
            ...
            if not service_info.is_available:
                logger.warning("服务 %s 当前不可用，状态: %s", service_info.name, service_info.state)
            ...
            try:
                ...
                if event_mod:
                    ...
                    if has_meta_param:
                        ...
                        result = await func(e)
                    return ServiceResult.success_result(result, time.time() - start_time)
                ...
                if isinstance(data, dict):
                    ...
                    missing_kwonly = [p for p in effective_kwonly_params if p not in data]
                    ...
                    if missing_pos or (missing_kwonly and not has_var_kw):
                        ...
                        return ServiceResult.error_result(error_msg, time.time() - start_time)
                    ...
                    for k, v in data.items():
                        if k in sig.parameters and k != 'meta':
                            ...
                            if not _isinstance_safe(v, param.annotation):
                                type_errors[k] = f"期望 {param.annotation}, 收到 {type(v).__name__}"
                    ...
                    try:
                        ...
                        return ServiceResult.error_result(f"调用异常: {ex}", time.time() - start_time)
                ...
                return ServiceResult.error_result(f"服务内部错误: {ex}", time.time() - start_time)
...
def service_pack(
    ...
    """
    @wraps(func)
    def _wrapper_factory(service_info: ServiceInfo, logger: Logger) -> Callable[[Event], Any]:
        """
        包装器工厂 - 二级包装
...
class ServiceMixin(PluginMixin):
    ...
    def _update_service_stats(self, service_name: ServiceName, results: Dict[UUID, Any]) -> None:
        """更新服务统计信息"""
    ...
    def discover_services(
        ...
        with self.__lock:
            ...
            # 过滤条件
            if pattern:
                services = [s for s in services if pattern in s.name or pattern in s.addr]
            if owner:
...
class ServiceResolver(LazyDecoratorResolver):
    ...
    def handle(self, plugin: Plugin | ServiceMixin, func: Callable, event_bus: EventBus) -> None:
        """处理服务注册逻辑"""
        try:
            # 从命名空间获取元数据
