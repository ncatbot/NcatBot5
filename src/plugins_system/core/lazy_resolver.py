...
from typing import TYPE_CHECKING, Any, Callable, ClassVar, Dict, List, Optional, Set, Tuple, Type, Union
...
class LazyDecoratorResolver(ABC, metaclass=_LazyResolverMeta):
    ...
    def has_all_attrs(data: Dict[str, Any], *specs: Tuple[str, Type]) -> bool:
        """增强的属性检查工具"""
        return all(
            k in data and type(data[k]) is t
            for k, t in specs
        )
...
# -------------------- 基础设施 --------------------
def tagged_decorator(tag: str, *, space: str = "default", **meta) -> Callable[[Callable], Callable]:
    """
    增强的标签装饰器工厂
