...
import sys
import traceback
from typing import Awaitable, Callable, Dict, Any, Final, Iterable, List, Optional, Type, Set, Tuple, Union
...
from ..utils.helpers import run_any
from ..core.lazy_resolver import LazyDecoratorResolver
from ..logger import logger
from ..utils.constants import NAMESPACE, PluginState, PROTOCOL_VERSION
...
from ..utils.constants import PluginSourceType, FeatureFlags
...
class PluginMeta(ABCMeta):
    ...
    def __init__(cls, name: str, bases: Tuple[type, ...], attrs: Dict[str, Any]) -> None:
        ...
        for base in bases:
            if (inspect.isclass(base) and 
                issubclass(base, PluginMixin) and 
                base is not PluginMixin):
                cls._mixins.append(base)
        
        # 从类属性中收集混入类
        for attr_name, attr_value in attrs.items():
            if (isinstance(attr_value, type) and 
                issubclass(attr_value, PluginMixin) and 
                attr_value is not PluginMixin and
                attr_value not in cls._mixins):
...
class Plugin(ABC, metaclass=PluginMeta):
    ...
    def __init__(self, context: PluginContext, config: Dict[str, Any], debug: bool = False):
        """初始化插件
    ...
    def _setup_mixins(self) -> None:
        ...
        for mixin_class in self._mixins:
            if isinstance(self, mixin_class):
                # 调用混入类的初始化方法
                if hasattr(mixin_class, '__init__') and mixin_class.__init__ is not object.__init__:
            ...
            for resolver in LazyDecoratorResolver.get_all_resolvers():
                if resolver.check(self, attr_value, event_bus):
                    try:
                        ...
                        self.logger.debug(f"应用装饰器: {resolver.__class__.__name__} -> {attr_name}")
                    except Exception as e:
