...
import inspect
import logging
from typing import Any, Awaitable, List, Optional, Set, Tuple, Union, Pattern, Dict
...
from ..utils.types import EventInterceptor, EventHandler, PluginName
...
class Event:
    ...
    if DEBUG_MODE:
        creation_stack: Optional[str] = field(default_factory=lambda: ''.join(traceback.format_stack(limit=6)))
    else:
...
class EventHandlerInfo:
    ...
    def matches_event(self, event_name: str) -> bool:
        ...
        """
        if self.is_regex:
            return self.event_pattern.match(event_name) != None
        else:
...
class EventBusBase(EventBus):
    ...
    def register_handler(
        ...
        logger.debug(f"[{handler.__class__.__name__}] 注册 handler: {pattern} -> {handler_id}")
        return handler_id
    ...
    def register_handlers(
        ...
        Returns:
            处理器ID字典
        """
        return {event: self.register_handler(event, handler, plugin_name) 
    ...
    def _get_matching_handlers(self, event_name: str) -> List[EventHandlerInfo]:
        ...
        with self._lock:
            return [
                handler_info for handler_info in self._handlers.values()
    ...
    @abstractmethod
    def _dispatch(self, handler: EventHandler, event: Event) -> Union[Future, Awaitable[Any]]:
        """调度处理器执行
