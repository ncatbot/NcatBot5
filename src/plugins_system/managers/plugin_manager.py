...
from ..core.plugins import Plugin, PluginStatus, PluginState, PluginSource
from ..utils.types import PluginName
from ..utils.constants import PROTOCOL_VERSION, SystemEvents, DEBUG_MODE
from ..utils.helpers import _version_satisfies
from ..exceptions import PluginDependencyError, PluginVersionError, PluginRuntimeError
...
class DefaultPluginManager(PluginManager):
    ...
    def _register_system_event_handlers(self) -> None:
        ...
        self.event_bus.register_handler(
            SystemEvents.MANAGER_STARTING,
            self._handle_manager_starting
        )
        self.event_bus.register_handler(
            SystemEvents.MANAGER_STOPPING,
            self._handle_manager_stopping
        )
        self.event_bus.register_handler(
            SystemEvents.RELOAD_REQUESTED,
            self._handle_reload_requested
        )
        self.event_bus.register_handler(
    ...
    async def _reload_all_plugins(self) -> None:
        ...
        for plugin_name, state in plugin_states.items():
            ...
            if plugin:
                if state == PluginState.RUNNING and plugin.status.state != PluginState.RUNNING:
    ...
    async def load_plugins(self) -> List[Plugin]:
        ...
        for source in sources:
            try:
                ...
                for plugin in plugins:
                    await self._send_plugin_event("discovered", plugin.name, plugin.meta)
                    
            except Exception as e:
                logger.error(f"加载插件失败 {source.path}:\n{e}")
                plugin_name = source.module_name
                await self._send_system_event(SystemEvents.LOAD_ERROR, {
                    "plugin": plugin_name,
        ...
        try:
            ...
            await self._send_system_event(SystemEvents.DEPENDENCY_RESOLVED, {
                "sorted_plugins": [p.name for p in sorted_plugins]
            })
        except PluginDependencyError as e:
            logger.error(f"插件依赖解析失败:\n{e}")
            await self._send_system_event(SystemEvents.DEPENDENCY_ERROR, {
                "error": str(e),
        ...
        for plugin in sorted_plugins:
            ...
            try:
                ...
                await self._send_system_event(SystemEvents.PLUGIN_LOADED, {
                    "plugin": plugin.name,
        ...
        if success_plugins:
            await self._send_system_event(SystemEvents.ALL_PLUGINS_LOADED, {
        ...
        await self._send_system_event(SystemEvents.MANAGER_STARTED, {
            "loaded_count": len(success_plugins)
        })
    ...
    async def unload_plugin(self, plugin_name: PluginName) -> bool:
        ...
        try:
            ...
            await self._send_system_event(SystemEvents.PLUGIN_UNLOADED, {
                "plugin": plugin_name
            })
            ...
            await self._send_system_event(SystemEvents.RUNTIME_ERROR, {
                "plugin": plugin_name,
                "operation": "unload",
                "error": str(e)
            })
    ...
    async def start_plugin(self, plugin_name: PluginName) -> bool:
        ...
        if plugin.status.state == PluginState.STOPPED:
            try:
                ...
                await self._send_system_event(SystemEvents.PLUGIN_STARTED, {
                    "plugin": plugin_name
                })
    ...
    async def stop_plugin(self, plugin_name: PluginName) -> bool:
        ...
        if plugin.status.state == PluginState.RUNNING:
            try:
                ...
                await self._send_system_event(SystemEvents.PLUGIN_STOPPED, {
                    "plugin": plugin_name
                })
