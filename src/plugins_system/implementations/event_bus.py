...
from ..core.events import Event, EventBusBase
...
class SimpleEventBus(EventBusBase):
    ...
    def __init__(self, max_workers: Optional[int] = None) -> None:
        ...
        self._executor = (
            ThreadPoolExecutor(
                max_workers=max_workers, thread_name_prefix="SimpleEventBus"
            )
            if max_workers
            else None
        )
    ...
    def publish(
        ...
        *,
        ...
        # 创建异步任务执行拦截器和处理器
        asyncio.create_task(
            self._async_publish(original_event), name=f"Publish-{event}"
        )
...
class NonBlockingEventBus(EventBusBase):
    ...
    def __init__(
        self, max_workers: Optional[int] = None, queue_maxsize: int = 10_000
    ) -> None:
        """初始化非阻塞事件总线
        ...
        self._executor = (
            ThreadPoolExecutor(
                max_workers=max_workers, thread_name_prefix="NonBlockingEventBus"
            )
            if max_workers
            else None
        )
        ...
        self._worker_thread = threading.Thread(
            target=self._run_worker_loop, name="NonBlockingEventBus-Worker", daemon=True
        )
    ...
    def _dispatch(self, handler: EventHandler, event: Event) -> Future:
        ...
        return asyncio.run_coroutine_threadsafe(
            self._execute_handler(handler, event), self._worker_loop
        )
        ...
        *,
        ...
        try:
            asyncio.run_coroutine_threadsafe(
                self._event_queue.put(event_obj), self._worker_loop
            )
        except Exception as e:
    ...
    async def request(
        ...
        *,
        ...
        future = asyncio.run_coroutine_threadsafe(
            self._worker_request(original_event, timeout), self._worker_loop
        )
