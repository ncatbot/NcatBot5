...
from ...abc.protocol_abc import ProtocolABC, RawMessage, RawUser, RawGroup
from ...core.IM import Message, User, Group, MessageContent
from ...utils.typec import UserID, GroupID, MsgId
...
class NapcatProtocol(ProtocolABC):
    ...
    def _content_to_segments(self, content: MessageContent) -> List[dict]:
        ...
        from ...core.nodes import (
            TextNode, ImageNode, AtNode, ReplyNode, FileNode
        )
        ...
        for node in content.nodes:
            if isinstance(node, TextNode):
                ...
                segments.append({
                    "type": "at",
                    "data": {"qq": node.user_id}
                })
            
            elif isinstance(node, ReplyNode):
                segments.append({
                    "type": "reply",
                    "data": {"id": node.message_id}
                })
    ...
    def _parse_message_content(self, segments: List[dict]) -> MessageContent:
        ...
        from ...core.nodes import TextNode, ImageNode, AtNode
    ...
    async def set_group_admin(self, group_id: GroupID, user_id: UserID, is_admin: bool = True) -> bool:
        """设置/取消管理员"""
    ...
    async def kick_group_member(self, group_id: GroupID, user_id: UserID, reason: Optional[str] = None) -> bool:
        """移除成员
    ...
    async def transfer_group_ownership(self, group_id: GroupID, new_owner_id: UserID) -> bool:
        """转让群主（napcat 不支持）"""
        # napcat 协议不支持转让群主
        return False
