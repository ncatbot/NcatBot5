...
class AsyncWebSocketClient(ABCWebSocketClient):
    ...
    async def get_message(self, listener_id: ListenerId, timeout: Optional[float] = None) -> Tuple[Any, MessageType]:
        """从监听器获取消息（异步阻塞）"""
        with self._listeners_lock:
    ...
    async def _evict_oldest_listener(self):
        ...
        if oldest_id:
            ...
            self.logger.warning(f"Evicted oldest listener due to max listeners: {oldest_id}")
...
class SyncWebSocketClient(ABCWebSocketClient):
    ...
    def create_listener(self, buffer_size: Optional[int] = None) -> str:
        ...
        future = asyncio.run_coroutine_threadsafe(
            self._client.create_listener(buffer_size), 
            self._loop
        )
    ...
    def remove_listener(self, listener_id: str):
        """移除监听器（同步）"""
        future = asyncio.run_coroutine_threadsafe(
            self._client.remove_listener(listener_id), 
            self._loop
        )
        future.result(timeout=10)
    
    def get_message(self, listener_id: str, timeout: Optional[float] = None) -> Tuple[Any, MessageType]:
        """获取消息（同步阻塞）"""
        future = asyncio.run_coroutine_threadsafe(
            self._client.get_message(listener_id, timeout), 
            self._loop
        )
    ...
    def send(self, message: Union[str, bytes, Dict]):
        """发送消息（同步）"""
        future = asyncio.run_coroutine_threadsafe(self._client.send(message), self._loop)
        future.result(timeout=10)
    
    def get_metrics(self) -> Dict[str, Any]:
        """获取客户端指标（同步）"""
        future = asyncio.run_coroutine_threadsafe(self._client.get_metrics(), self._loop)
        return future.result(timeout=10)
