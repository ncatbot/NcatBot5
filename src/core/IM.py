...
class Message:
    ...
    async def forward_to_group(self, group: Group | GroupID) -> "Message":
        ...
        forward_content = MessageContent(nodes=[
            ForwardNode(
                message_id=self._id,
                sender_id=self._sender_id,
...
class Group:
    ...
    async def set_admin(self, user: Union["User", UserID], is_admin: bool = True) -> bool:
        """设置/取消管理员"""
    ...
    async def remove_member(self, user: Union["User", UserID], reason: Optional[str] = None) -> bool:
        """移除成员"""
        user_id = user.uid if isinstance(user, User) else user
