...
import mimetypes
import io
from typing import Literal, Union, Optional, Dict, Any, AsyncIterator, BinaryIO
...
class CacheManager:
    ...
    def __init__(self, 
        ...
        logger.info(f"CacheManager initialized: dir={self.cache_dir}, max_size={self._format_size(max_size)}, ignore_limit={ignore_limit}")
    ...
    async def aput(self, 
                  ...
                  cache_policy: CachePolicy = CachePolicy.DEFAULT) -> Optional[Path]:
        """
        异步保存到缓存
    ...
    def _cleanup_space(self, required_size: int) -> bool:
        ...
        for file_info in files_with_info:
            ...
            try:
                ...
                # 从访问时间记录中删除
                key = file_info['path'].name.split('_')[0] if '_' in file_info['path'].name else file_info['path'].name
                if key in self.access_times:
                    del self.access_times[key]
                
                logger.debug(f"Cleaned cache file: {file_info['path'].name} ({self._format_size(file_info['size'])})")
            except Exception as e:
        ...
        if current_size + required_size <= self.max_size:
            ...
            logger.warning(f"Failed to free enough space. Current: {self._format_size(current_size)}, "
    ...
    def clear(self):
        ...
        try:
            ...
            logger.info(f"Cache cleared: {deleted_count} files, {self._format_size(deleted_size)}")
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.access_times:
            ...
            if timestamps:
                oldest_access = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(min(timestamps)))
                newest_access = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(max(timestamps)))
...
class Resource:
    ...
    def __init__(self, 
        ...
        logger.debug(f"Resource initialized: uri={uri}, is_url={self._is_url}, use_cache={use_cache}, cache_policy={cache_policy}")
...
# 工厂函数
async def resource_open(uri: str,
                       mode: str = 'rb',
                       session: Optional[aiohttp.ClientSession] = None,
                       cache_manager: Optional[CacheManager] = None,
                       use_cache: bool = True,
                       cache_policy: CachePolicy = CachePolicy.DEFAULT,
                       timeout: int = DEFAULT_TIMEOUT,
                       chunk_size: int = DEFAULT_CHUNK_SIZE) -> Resource:
    """
    打开资源（类似open()函数）
    ...
    Returns:
        Resource对象
    """
    return Resource(uri, mode, session, cache_manager, use_cache, cache_policy, timeout, chunk_size)
...
class ResourceFetcher:
    ...
    async def fetch(self, uris: list, 
                   use_cache: bool = True,
                   cache_policy: Optional[CachePolicy] = None) -> list:
        """批量获取资源"""
