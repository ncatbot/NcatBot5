...
if __name__ == "__main__":
    ...
    try:
        strict_dto = StrictDTO(name="Alice", email="alice@example.com", age=30)
        print(f"✓ 正确创建StrictDTO: {strict_dto.name}")
    ...
    print("\n2. 使用上下文管理器临时禁用类型检查:")
    try:
        with strict_dto.type_check_context:
            print("  进入上下文管理器")
            strict_dto.age = "thirty"  # 临时允许
            print(f"  ✓ 临时设置字符串值: {strict_dto.age}")
            print("  退出上下文管理器")
        print("✓ 上下文管理器正常退出")
    ...
    # 验证字段
    print("\n3. 验证字段:")
    print(f"  当前age值: {strict_dto.age} (类型: {type(strict_dto.age)})")
    print(f"  验证结果: {strict_dto.validate()}")

    # 测试恢复后的类型检查
    print("\n4. 恢复后的类型检查:")
    try:
        strict_dto.age = "forty"  # 应该再次触发类型检查
        print("✗ 应该捕获类型错误但没有")
    except TypeError as e:
        print(f"✓ 正确捕获类型错误: {e}")
    ...
    try:
        config = ConfigDTO(api_url="https://api.example.com", timeout=60)
        print("✓ 正确创建ConfigDTO")
        ...
        # 测试序列化
        print("\n  JSON序列化:")
        print(f"  {config.to_json(indent=2)}")
    ...
    class NetworkConfigDTO:
        ...
        endpoints: Dict[str, URL]
        allowed_ips: List[IPAddress]
    ...
    try:
        ...
        print("✓ 正确创建NetworkConfigDTO")
        print(f"  Server: {network_config.server_name}")
        print(f"  Primary IP: {network_config.primary_ip}")
        print(f"  Endpoints: {list(network_config.endpoints.keys())}")

    except (TypeError, ValueError) as e:
        print(f"✗ 创建失败: {e}")

    print("\n=== 异常类型总结 ===")
    print("TypeError: 当值不是期望的类型时")
    print("ValueError: 当值是正确的类型但格式/内容不合法时")
    print("示例:")

    # TypeError示例
    try:
        bad_config = ConfigDTO(api_url=123, timeout=30)  # TypeError: URL期望str，得到int
    except TypeError as e:
        print(f"  TypeError示例: {e}")

    # ValueError示例
    try:
        bad_config = ConfigDTO(
            api_url="not-a-valid-url", timeout=30  # ValueError: URL格式无效
        )
    except ValueError as e:
        print(f"  ValueError示例: {e}")
